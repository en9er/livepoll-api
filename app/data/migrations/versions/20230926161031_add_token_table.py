...
def upgrade() -> None:
    file = Path(f"./app/data/migrations/versions/sqls/{revision}_add-token-table/UP.sql")
    op.execute(sqltext=file.read_text())


def downgrade() -> None:
    file = Path(f"./app/data/migrations/versions/sqls/{revision}_add-token-table/DOWN.sql")
    op.execute(sqltext=file.read_text())
